...
describe Parade::Parsers::JsonFileParser do
  ...
  describe ".parse" do
    ...
    let(:filepath) { "parade.json" }

    let(:file_contents) { mock('parade json contents') }
    ...
    it "should load the file, convert it to the DSL format, and parse it" do
      ...
      described_class.should_receive(:convert_to_parade_dsl).with(file_contents).and_return(dsl_contents)
      Parade::Parsers::Dsl.should_receive(:parse).with(dsl_contents,expected_options)
  ...
  describe ".convert_to_parade_dsl" do
    ...
    context "when given a hash of sub-sections" do
      ...
      it "should generate the correct data for the DSL to parse" do
        described_class.convert_to_parade_dsl(contents).should eq expected
      end
